            ...
            {order && !loading && (
              <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${statusColor(order.status)}`}>
                    {fixStatusDescription(order.statusDescription || order.status)}
                  </span>
                  <span className="text-xs text-gray-400">Creado: {fmtDate(order.creationDate)}</span>
                  {order.invoicedDate && (
                    <span className="text-xs text-gray-400">Facturado: {fmtDate(order.invoicedDate)}</span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {order.totals?.map(t => (
                    <div key={t.id} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                      ...
                      <p className={`text-sm font-bold ${t.value < 0 ? 'text-red-600' : 'text-gray-800'}`}>
                        {t.value < 0
                  ...
                  {order.clientProfileData && (
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                      ...
                      <div className="space-y-1.5 text-sm">
                        ...
                        <div className="flex justify-between">
                          <span className="text-gray-400 text-xs">{order.clientProfileData.documentType}</span>
                          <span className="text-gray-700 font-medium text-xs">{order.clientProfileData.document}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-400 text-xs">Teléfono</span>
                          <span className="text-gray-700 font-medium text-xs">{order.clientProfileData.phone || '—'}</span>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    {order.shippingData?.address && (
                      <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                        <div className="flex items-center gap-1.5 mb-3">
                          ...
                          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Envío</p>
                        </div>
                        ...
                        {order.paymentData.transactions[0].payments.map((p, i) => (
                          <div key={i} className="space-y-1">
                            <div className="flex justify-between">
                              ...
                              <span className="text-xs font-medium text-gray-700">{p.paymentSystemName}</span>
                            </div>
                            <div className="flex justify-between">
                            ...
                            {p.installments > 1 && (
                              <div className="flex justify-between">
                                ...
                                <span className="text-xs text-gray-700">{p.installments}</span>
                              </div>
                            )}
                          </div>
                        ))}
                        ...
                        return (
                          <div key={item.uniqueId || i}
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-medium text-gray-700 leading-snug line-clamp-2">{item.name}</p>
